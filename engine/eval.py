...
from utils.vis import visualize_depth_numpy, visualize_palette_numpy
...
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, N_samples=-1, white_bg=False, ndc_ray=False,
    ...
    pbar = trange(len(test_rays), file=sys.stdout)
    for idx in pbar:
        ...
        if palette is not None and plt_map.shape[-1] > 3:
            opaque = rearrange(plt_map[..., 3:3 + palette.shape[0]], '(h w) c-> h w c', h=H, w=W).cpu()
            plt_decomp = visualize_palette_numpy(opaque.numpy(), palette.numpy())
